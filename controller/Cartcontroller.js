...
export const createCart = async (req, res) => {
    try {
        const { jumlah_barang, total_harga, BuahId } = req.body;
        
        if (!jumlah_barang || !total_harga || !BuahId) {
            return res.status(400).json({ message: 'Semua field harus diisi' });
        }

        const newCart = await Cart.create({ jumlah_barang, total_harga, BuahId });
...
export const updateCart = async (req, res) => {
    try {
        const { jumlah_barang, total_harga, BuahId } = req.body
        const data = await Cart.update({jumlah_barang, total_harga, BuahId }, {
            where: {
                id: req.params.id
            }
        })
